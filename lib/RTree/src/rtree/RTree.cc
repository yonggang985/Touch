	...
	id_type& indexIdentifier)
{
	...
	{
	case BLM_STR:
		bl.bulkLoadUsingSTR(static_cast<RTree*>(tree), stream, bindex, bleaf, 10000, 600);
		break;
	case BLM_HIL:
		bl.bulkLoadUsingHilbert(static_cast<RTree*>(tree), stream, bindex, bleaf, 10000, 600);
		break;
	case BLM_PRI:
		bl.bulkLoadUsingPR(static_cast<RTree*>(tree), stream, bindex, bleaf, 10000, 600);
		break;
	default:
		throw Tools::IllegalArgumentException("createAndBulkLoadNewRTree: Unknown bulk load method. " + m);
		break;
	}
	...
	id_type& indexIdentifier)
{
	...
	double fillFactor = 0;
	uint32_t indexCapacity = 0, leafCapacity = 0, dimension = 0, pageSize = 0, numberOfPages = 0;
	...
	case BLM_STR:
		...
		break;
	case BLM_HIL:
		bl.bulkLoadUsingHilbert(static_cast<RTree*>(tree), stream, bindex, bleaf, pageSize, numberOfPages);
		break;
	case BLM_PRI:
		bl.bulkLoadUsingPR(static_cast<RTree*>(tree), stream, bindex, bleaf, pageSize, numberOfPages);
		break;
	default:
...
void SpatialIndex::RTree::RTree::seedQuery(const IShape& query, IVisitor& v)
{
	if (query.getDimension() != m_dimension) throw Tools::IllegalArgumentException("seedQuery: Shape has the wrong number of dimensions.");
	seedRangeQuery(query, v);
}

void SpatialIndex::RTree::RTree::intersectsWithQuery2(const Region& query, IVisitor& v)
{
	if (query.getDimension() != m_dimension) throw Tools::IllegalArgumentException("seedQuery: Shape has the wrong number of dimensions.");
	rangeQuery(IntersectionQuery, query, v);
}
...
SpatialIndex::RTree::NodePtr SpatialIndex::RTree::RTree::readNode(id_type page)
{
	...
	try
	{
		...
		m_stats.m_u64ByteReads += dataLength;
	}
	catch (InvalidPageException& e)
	{
		std::cerr << e.what() << std::endl;
...
void SpatialIndex::RTree::RTree::seedKNN(const Point& query, IVisitor& v)
{
	if (query.m_dimension != m_dimension) throw Tools::IllegalArgumentException("pointLocationQuery: Shape has the wrong number of dimensions.");
	Region r(query, query);
	seedRangeQuery(r, v);
...
void SpatialIndex::RTree::RTree::seedRangeQuery(const IShape& query, IVisitor& v)
{
#ifdef PTHREADS
	Tools::SharedLock lock(&m_rwLock);
#else
	...
	try
	{
		...
		while (! st.empty())
		{
			...
			if (n->m_level == 0)
			{
				v.visitNode(*n);
				for (uint32_t cChild = 0; cChild < n->m_children; ++cChild) {
					if ( query.intersectsShape(*(n->m_ptrMBR[cChild])) || query.containsShape(*(n->m_ptrMBR[cChild])) )
					{
						Data data = Data(n->m_pDataLength[cChild], n->m_pData[cChild], *(n->m_ptrMBR[cChild]), n->m_pIdentifier[cChild]);
						if (seeded==false)
							{
							v.visitData(data, n->m_identifier);
							++(m_stats.m_u64QueryResults);
							if(v.doneVisiting()) {
								seeded=true;
							}
...
void SpatialIndex::RTree::RTree::rangeQuery(RangeQueryType type, const Region& query, IVisitor& v)
{
#ifdef HAVE_PTHREAD_H
	Tools::SharedLock lock(&m_rwLock);
#else
	...
	try
	{
		...
		if (root->m_children > 0 && query.intersectsRegion(root->m_nodeMBR)) st.push(root);
		...
		while (! st.empty())
		{
			...
			if (n->m_level == 0)
			{
				...
				for (uint32_t cChild = 0; cChild < n->m_children; ++cChild)
				{
					...
					else b = query.intersectsRegion(*(n->m_ptrMBR[cChild]));
			...
			else
			{
				...
				for (uint32_t cChild = 0; cChild < n->m_children; ++cChild)
				{
					if (query.intersectsRegion(*(n->m_ptrMBR[cChild]))) st.push(readNode(n->m_pIdentifier[cChild]));
				}
			}
		}
