...
{
    SpatialIndex::id_type new_page
        = serialize_vertices (v_start, v_end, storage);
                 ...
                 bool force_remainder)
{
    ...
    {
        if (do_outp)
            outp << v_ptr->first << ' ' << v_ptr->second << '\n';
#ifndef NDEBUG
        std::cerr << "vtx size = " << v_ptr->second.serialized_size () << '\n';
#endif
        total_bytes += v_ptr->second.serialized_size ();
        if (total_bytes > dias::default_page_size)
        {
#ifdef VERBOSE
            std::cerr << "Full page made!\n";
#endif
            output_page_of_vertices (v_start, v_ptr, vertices, storage,
                                     v_id_to_page);
            total_bytes = sizeof (uint32_t) + v_ptr->second.serialized_size ();
            v_start = v_ptr;
        }
        v_ptr++;
    }
#ifdef VERBOSE
    std::cerr << "Only " << total_bytes << " for the last page\n";
#endif
    if (force_remainder)
    {
#ifdef VERBOSE
        std::cerr << "Outputing the last page anyway\n";
#endif
        output_page_of_vertices (v_start, v_ptr,  vertices, storage,
                                 v_id_to_page);
    }
