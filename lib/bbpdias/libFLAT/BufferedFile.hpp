...
namespace FLAT
{
	...
	public:
		...
		void read(uint32 size, uint8* buffer);
		void write(uint32 size, uint8* buffer);
	...
	inline void BufferedFile::read(uint32 size, uint8* buffer)
	{
	#ifdef FATAL
		if (eof)
	...
	inline void BufferedFile::write(uint32 size, uint8* buffer)
	{
		try
		{
