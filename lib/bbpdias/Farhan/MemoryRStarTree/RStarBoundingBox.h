...
struct RStarBoundingBox {
	...
	std::pair<float, float> edges[dimensions];
	...
	void reset()
	{
		...
		{
			edges[axis].first = std::numeric_limits<float>::max();
			edges[axis].second = std::numeric_limits<float>::min();
		}
	}
	...
	double overlap(const RStarBoundingBox<dimensions>& bb) const
	{
		...
		for (std::size_t axis = 0; area && axis < dimensions; axis++)
		{
			...
			const float x1 = edges[axis].first;
			const float x2 = edges[axis].second;
			const float y1 = bb.edges[axis].first;
			const float y2 = bb.edges[axis].second;
